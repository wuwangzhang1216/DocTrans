...
class DocumentTranslator {
  constructor() {
    ...
    // On Heroku, the backend directory is the root, so translators are in ./translators
    // In local dev, backend is at translate-web-app/backend, so need ../..
    // Check if translators exists locally first
    const localTranslators = path.join(__dirname, 'translators');
    const parentTranslators = path.join(__dirname, '../..');

    // Use local translators if it exists (Heroku), otherwise use parent (local dev)
    this.projectRoot = require('fs').existsSync(localTranslators) ? __dirname : parentTranslators;
  }
